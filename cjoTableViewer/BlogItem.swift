...
//
//   let blogItem = try? newJSONDecoder().decode(BlogItem.self, from: jsonData)
//
// To read values from URLs:
//
//   let task = URLSession.shared.blogItemTask(with: url) { blogItem, response, error in
//     if let blogItem = blogItem {
//       ...
//     }
//   }
//   task.resume()
...
typealias BlogItem = [BlogItemElement]

struct BlogItemElement: Codable {
    let id: Int
...
extension URLSession {
    ...
    func blogItemTask(with url: URL, completionHandler: @escaping (BlogItem?, URLResponse?, Error?) -> Void) -> URLSessionDataTask {
